...
defineEmits(["updateCount", "clear", "addOneItem", "clearOneItem"]);
</script>
<template>
  <tr class="text-sm">
    <td class="py-4">
      <div class="flex items-center">
        <span class="font-semibold">{{ product.nickname }}</span>
      </div>
    </td>
    <td class="">
      {{ product.description }}
    </td>
    <td class="py-4 text-center">{{ product.price_net }}€<br /></td>
    <td class="py-4">
      <div class="flex items-center justify-center">
        <button
          class="border rounded-md py-2 px-4 mr-2"
          @click="$emit('clearOneItem')"
        >
          -
        </button>
    ...
    <td class="py-4 text-center">
      {{ (count * product.price_net).toFixed(2) }}€
    </td>
    <td>
